...
public class ActionBar extends RelativeLayout {
    ...
    private final View.OnLongClickListener mActionLongClicked = new View.OnLongClickListener() {
    	@Override
     ...
     */
    public ActionBar(BA ba, AttributeSet attrs, Drawable background, String eventName) {
...
//        TypedArray a = ba.context.obtainStyledAttributes(attrs,
//                RHelper.getResourceArray("styleable", "ActionBar"));
//        final int N = a.getIndexCount();
//        for (int i = 0; i < N; i++) {
//            int attr = a.getIndex(i);
//            if (attr == RHelper.getResourceId("styleable", "ActionBar_title")) {
...
//        this.setLayoutParams(params);
        ...
        mActionWidth = (int) getResources().getDimension(RHelper.getResourceId("dimen", "actionbar_item_width"));
        
        mInflater = (LayoutInflater) ba.context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);

        mBarView = (LinearLayout) mInflater.inflate(RHelper.getResourceId("layout", "ahab_actionbar"), null);
        addView(mBarView);

        mBackground = (RelativeLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_background"));
        mHomeView = (FrameLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_home_view"));
        mHomeLogo = (ImageView) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_home_logo"));
        mHomeUpIndicator = mBarView.findViewById(RHelper.getResourceId("id", "actionbar_home_is_back"));

        mTitleView = (TextView) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_title"));
        mSubtitleView = (TextView) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_subtitle"));
        
        mListView = (FrameLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_list"));
        mListIndicator = mBarView.findViewById(RHelper.getResourceId("id", "actionbar_list_indicator"));
        
        mCustomView = (FrameLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_custom"));
        
        mActionsView = (LinearLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_actions"));

        mTabsView = (LinearLayout) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_tabs"));

        mProgress = (ProgressBar) mBarView.findViewById(RHelper.getResourceId("id", "actionbar_progress"));
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            Drawable logo = LogoLoader.loadLogo(ba.context);
            if (logo != null) {
                setHomeLogo(logo);
            }
        }
        ...
        if (background != null) {
        	...
        	mHomeLogo.setBackgroundDrawable(background);
        }
    ...
    public void setBackgroundDrawable(Drawable background) {
    	if (background != null) {
    		...
    		mBackground.setBackgroundDrawable(mBackgroundDrawable.getConstantState().newDrawable());
    		mHomeLogo.setBackgroundDrawable(mBackgroundDrawable.getConstantState().newDrawable());
    		mHomeView.setBackgroundDrawable(mBackgroundDrawable.getConstantState().newDrawable());
    		//mBarView.setBackgroundDrawable(mBackgroundDrawable.getConstantState().newDrawable());
    		
    		//mProgress.setBackgroundDrawable(mBackgroundDrawable.getConstantState().newDrawable());
    		
            for (int i = 0; i < getActionCount(); i++) {
                Action item = getActionAt(i);
                item.setBackgroundDrawable(mBackgroundDrawable);
            }
            
            Action home = findAction(mHomeID);
            if (home != null)
            	home.setBackgroundDrawable(background);
    	}
    }
    ...
    public void setActionWidth(int width) {
    	...
    	for (int i = 0; i < getActionCount(); i++) {
    		...
    		ViewGroup.LayoutParams params = item.mView.getLayoutParams();
    		params.width = width;
    		
    		item.mView.setLayoutParams(params);
    	}
    	
    	Action home = findAction(mHomeID);
    	if (home != null) {
    		ViewGroup.LayoutParams params = home.mView.getLayoutParams();
    		params.width = width;
    		
    		home.mView.setLayoutParams(params);
    	}
    }
    ...
    public Action findAction(int itemId) {
    	if (itemId == mHomeID)
    		itemId = RHelper.getResourceId("id", "actionbar_item_home");
     ...
     */
    public void setHomeAction(Action action, int itemID) {
    ...
    public static class Action implements MenuItem {
        ...
        Action(ActionBar actionBar) {
            ...
            mBackground = actionBar.mBackgroundDrawable;
            
            mView = mActionBar.mInflater.inflate(RHelper.getResourceId("layout", "ahab_item"), mActionBar.mActionsView, false);
            mView.setTag(this);
            mView.setOnClickListener(mActionBar.mActionClicked);
            mView.setOnLongClickListener(mActionBar.mActionLongClicked);
            ViewGroup.LayoutParams params = mView.getLayoutParams();
            params.width = mActionBar.mActionWidth;
            mView.setLayoutParams(params);
            
            mIcon = (ImageButton) mView.findViewById(RHelper.getResourceId("id", "actionbar_item"));
            if (mBackground != null)
            	mIcon.setBackgroundDrawable(mBackground.getConstantState().newDrawable());
        }
        ...
        Action(ActionBar actionBar, int groupId, int itemId, int order, CharSequence title) {
            ...
            int layoutId = (itemId == RHelper.getResourceId("id", "actionbar_item_home")) ? RHelper.getResourceId("layout", "ahab_item_home") : RHelper.getResourceId("layout", "ahab_item");
        ...
        public final Action setIconPadding(int dip) {
        	mIcon.setPadding(dip, dip, dip, dip);
        	return this;
        }
        ...
        public final Action setHelpText(CharSequence text) {
        	mHelpText = text;
        	return this;
        }
        ...
        public void select() {
            ...
            if (mActionBar.mEventName != null) {
            	if (mActionBar.mBa.subExists(mActionBar.mEventName + "_itemclicked")) {
            		int id = this.getItemId();
            		if (id == RHelper.getResourceId("id", "actionbar_item_home")) {
            			id = mActionBar.mHomeID;
            		}
            		mActionBar.mBa.raiseEvent(this, mActionBar.mEventName + "_itemclicked", new Object[] {id} );
            	}
            }
            if (mIntent != null) {
        ...
        public void showHelp() {
        	if (mHelpText != null)
        		Toast.makeText(mActionBar.mContext, mHelpText, Toast.LENGTH_SHORT).show();
        }
    ...
    public final class Tab {
        ...
        Tab() {
            mView = mInflater.inflate(RHelper.getResourceId("layout", "ahab_tab"), mTabsView, false);
            mView.setTag(this);
            mView.setOnClickListener(mTabClicked);
            
            mIconView = (ImageView)mView.findViewById(RHelper.getResourceId("id", "actionbar_tab_icon"));
            mTextView = (TextView)mView.findViewById(RHelper.getResourceId("id", "actionbar_tab_text"));
            mCustomView = (FrameLayout)mView.findViewById(RHelper.getResourceId("id", "actionbar_tab_custom"));
        }
